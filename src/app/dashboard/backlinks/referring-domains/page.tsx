...
export default async function RefDomainsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (historyId) {
    ...
    const dedupeId = stableSearchId(['ref-domains', target, limit]);
    const cached = getRefDomainsResults<RefDomain>(dedupeId);
    if (cached) {
      items = cached;
      const cachedEntry = getRefDomainsHistory().find((h) => h.id === dedupeId);
      ...
      try {
        ...
        if (result.error) {
          ...
          total = result.total;
          cost = result.cost;
          saveRefDomainsSearch({ id: dedupeId, ts: Date.now(), target, cost, total }, items);
        }
      } catch (e) {
        error = String(e);
      }
  ...
  return (
    <div className="space-y-6 pb-12">
      <div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Referring Domains</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium">Domains linking to a target</p>
      </div>
          ...
          {items.length > 0 && (
            <div className="space-y-3">
              ...
              <div id="results" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden">
                <ReferringDomainsTable items={items} />
              </div>
            </div>
          )}
        </div>
